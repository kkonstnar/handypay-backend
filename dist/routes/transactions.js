        ...
        const mainLogic = async () => {
            ...
            try {
                ...
                try {
                    ...
                    console.error("❌ Database query failed, attempting retry:", dbQueryError);
                    // Force database reconnection and retry once
                    const dbModule = await import("../db.js");
                    if (dbModule &&
                        typeof dbModule.resetDatabaseConnection === "function") {
                ...
                const formattedTransactions = userTransactions.map((tx) => ({
                    ...
                    completedAt: tx.status === "completed" && tx.updatedAt
                        ? new Date(tx.updatedAt)
                        : undefined, // When transaction was completed
                }));
                console.log("✅ Returning formatted transactions:", formattedTransactions.length);
                return c.json({
                    success: true,
