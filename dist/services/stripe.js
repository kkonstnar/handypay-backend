...
import { sendPaymentReceivedNotification, sendWelcomeNotification, } from "./push-notifications.js";
...
export class StripeService {
    static async createAccountLink(env, { userId, account_id, firstName, lastName, email, refresh_url, return_url, }) {
        try {
            ...
            if (accountStatus.details_submitted && accountStatus.charges_enabled) {
                ...
                const defaultRefreshUrl = "https://handypay-backend.handypay.workers.dev/api/stripe/refresh";
                const defaultReturnUrl = "https://handypay-backend.handypay.workers.dev/api/stripe/return";
                let validRefreshUrl = refresh_url;
                let validReturnUrl = return_url;
                try {
                    if (!refresh_url || !refresh_url.startsWith("http")) {
                        console.log("⚠️ Invalid or missing refresh_url for update, using default");
                        validRefreshUrl = defaultRefreshUrl;
                    }
                    if (!return_url || !return_url.startsWith("http")) {
                        console.log("⚠️ Invalid or missing return_url for update, using default");
    ...
    static async transferExistingFundsToConnectedAccount(env, userId) {
        try {
            console.log(`💸 Transferring existing platform funds to connected account for user: ${userId}`);
            const stripe = getStripe(env);
            ...
            const availableBalance = platformBalance.available.find((balance) => balance.currency === "jmd");
            ...
            console.log(`✅ Transfer created: ${transfer.id}, amount: ${availableBalance.amount} cents`);
            return {
                success: true,
    ...
    static async createPaymentLink(env, { handyproUserId, customerName, customerEmail, description, amount, taskDetails, dueDate, currency = "USD", paymentSource = "payment_link_modal", }) {
        try {
            console.log(`💳 Creating payment link for user ${handyproUserId}, amount: ${amount} cents, currency: ${currency}`);
            const stripe = getStripe(env);
            ...
            console.log(`💳 Using payment methods for ${currency}: ${paymentMethodTypes.join(", ")}`);
            ...
            console.log(`💸 Using destination charges - funds will automatically transfer to: ${stripeAccountId}`);
            ...
            try {
                ...
                await db.insert(transactions).values({
                    ...
                    paymentMethod: paymentSource === "qr_generation" ? "qr_code" : "payment_link", // Set payment method based on source
            ...
            console.error("❌ Error creating Jamaican destination charges payment link:", error);
            ...
            if (error instanceof Error) {
                if (error.message.includes("payment method") ||
                    ...
                    throw new Error("Payment method configuration issue. Please check your Stripe account settings or contact support.");
                }
                else if (error.message.includes("destination")) {
                    throw new Error("Destination charges setup issue. Contact support to configure your Jamaican account properly.");
                }
                else if (error.message.includes("transfer")) {
                    throw new Error("Transfer configuration issue. Contact support to enable fund transfers for your account.");
                }
                else if (error.message.includes("capability") ||
                    error.message.includes("capabilities")) {
                    throw new Error("Account capability issue. Your Stripe account may need additional setup for destination charges.");
                }
                else if (error.message.includes("platform") ||
                    error.message.includes("application")) {
                    throw new Error("Platform configuration issue. The platform account needs destination charges enabled.");
                }
            }
            throw error;
        }
    ...
    static async handleWebhook(env, rawBody, signature) {
        try {
            const stripe = getStripe(env);
            // Try Cloudflare env first, then fall back to process.env for local development
            const STRIPE_WEBHOOK_SECRET = env?.STRIPE_TEST_WEBHOOK_SECRET ||
                process.env.STRIPE_TEST_WEBHOOK_SECRET ||
                env?.STRIPE_WEBHOOK_SECRET ||
                process.env.STRIPE_WEBHOOK_SECRET;
            if (!STRIPE_WEBHOOK_SECRET) {
                throw new Error("STRIPE_WEBHOOK_SECRET (or STRIPE_TEST_WEBHOOK_SECRET) is required");
            }
            ...
            console.log(`🔍 Webhook data preview:`, {
                type: event.type,
                id: event.data.object.id || "no-id",
                object: event.data.object.object,
                payment_link: event.data.object.payment_link || null,
        ...
        try {
            ...
            if (existingTransaction.length > 0) {
                ...
                try {
                    ...
                    if (fullPaymentIntent.payment_method &&
                        ...
                        if (fullPaymentIntent.payment_method.type === "card" &&
            ...
            try {
                ...
                if (userId) {
                    ...
                    console.log(`🔔 Attempting to send push notification to user ${userId} for payment ${paymentIntent.id}:`, {
                        amount: paymentIntent.amount,
                        currency,
                        customerName,
                    });
                    const notificationResult = await sendPaymentReceivedNotification(userId, paymentIntent.amount, currency, customerName, env);
                    console.log(`📋 Push notification result:`, notificationResult);
                    if (notificationResult.success) {
                        console.log(`📱 ✅ Push notification sent to user ${userId} for payment ${paymentIntent.id} (sent to ${notificationResult.sentTo} devices)`);
                    }
                    else {
                        console.warn(`⚠️ ❌ Failed to send push notification for payment ${paymentIntent.id}:`, notificationResult.error);
                    }
                }
                else {
                    console.warn(`⚠️ No user ID in payment metadata, skipping push notification`);
                }
            }
    ...
    static async handlePaymentIntentFailed(env, paymentIntent) {
        ...
        try {
            ...
            try {
                ...
                console.warn("⚠️ Could not retrieve payment method details for failed payment:", error);
            }
    ...
    static async handleAccountUpdated(env, account) {
        ...
        console.log("🔄 WEBHOOK: Full account data:", JSON.stringify({
            id: account.id,
            charges_enabled: account.charges_enabled,
            details_submitted: account.details_submitted,
            requirements: account.requirements,
        ...
        try {
            ...
            const user = await db
                ...
                .limit(1);
            console.log(`🔍 WEBHOOK: Found ${user.length} users for Stripe account ${account.id}`);
            if (user.length > 0) {
                ...
                if (onboardingCompleted) {
                    console.log(`🔄 WEBHOOK: Updating database for user ${userId} - setting stripeOnboardingCompleted: true`);
                    ...
                    console.log(`✅ WEBHOOK: Onboarding marked complete for user: ${userId}`);
                    ...
                    const verifyUser = await db
                        ...
                        .where(eq(users.id, userId))
                        .limit(1);
                    console.log(`🔍 WEBHOOK: Verification - user ${userId} onboarding status:`, verifyUser[0]?.stripeOnboardingCompleted);
                    // Send welcome notification to user
                    try {
                        const notificationResult = await sendWelcomeNotification(userId, user[0]?.fullName ?? user[0]?.firstName ?? undefined, env);
                        if (notificationResult.success) {
                            console.log(`🎉 Welcome notification sent to user ${userId}`);
                        }
                        else {
                            console.warn(`⚠️ Failed to send welcome notification to user ${userId}:`, notificationResult.error);
                        }
                    }
                    catch (notificationError) {
                    ...
                    console.log(`📡 Broadcasting onboarding completion to user ${userId}`);
                    ...
                    try {
                        ...
                        console.log(`✅ Onboarding completed - frontend can now poll for updated status for user ${userId}`);
                    }
                    catch (notifyError) {
            ...
            if (session.payment_link && session.payment_status === "paid") {
                ...
                if (existingTransaction.length > 0) {
                    ...
                    await db
                        ...
                        updatedAt: new Date(),
                        notes: metadata.paymentType === "destination_charges"
                        ...
                        .where(eq(transactions.stripePaymentLinkId, session.payment_link));
                    console.log(`✅ Updated ${metadata.paymentType || "direct"} payment link transaction to completed: ${transaction.id}`);
                    // For destination charges, log the transfer information
                    if (metadata.paymentType === "destination_charges") {
                        console.log(`💸 Destination charges payment - funds automatically transferred to: ${metadata.connectedAccountId}`);
                        console.log(`💰 Transfer amount: ${metadata.originalAmount || transaction.amount} ${metadata.originalCurrency || transaction.currency} cents`);
                    }
                    // Send push notification to user
                    const userId = transaction.userId;
                    // Use the actual currency from the transaction
                    const displayAmount = metadata.originalAmount || transaction.amount;
                    const displayCurrency = metadata.originalCurrency || transaction.currency || "USD";
                    const customerName = transaction.customerName || undefined;
                    try {
                        console.log(`🔔 Attempting to send push notification to user ${userId} for payment link ${session.payment_link}:`, {
                            displayAmount,
                            displayCurrency,
                            customerName,
                            paymentType: metadata.paymentType,
                        });
                        const notificationResult = await sendPaymentReceivedNotification(userId, displayAmount, displayCurrency, customerName, env);
                        console.log(`📋 Push notification result:`, notificationResult);
                        if (notificationResult.success) {
                            console.log(`📱 ✅ Push notification sent to user ${userId} for ${metadata.paymentType || "direct"} payment link ${session.payment_link} (sent to ${notificationResult.sentTo} devices)`);
                        }
                        else {
                            console.warn(`⚠️ ❌ Failed to send push notification for payment link ${session.payment_link}:`, notificationResult.error);
                        }
                    }
                    catch (notificationError) {
                        console.error("❌ Error sending payment link notification:", notificationError);
                        // Don't fail the webhook because of notification error
                    }
                }
                else {
        ...
        try {
            ...
            if (invoice.payment_link) {
                ...
                if (existingTransaction.length > 0) {
                    ...
                    try {
                        ...
                        if (invoice.payment_intent) {
                            ...
                            if (paymentIntent.payment_method &&
                                ...
                                // Extract card details if it's a card payment
                                if (paymentIntent.payment_method.type === "card" &&
                    ...
                    // For destination charges, log the transfer information
                    if (metadata.paymentType === "destination_charges") {
                        console.log(`💸 Destination charges invoice payment - funds automatically transferred to: ${metadata.connectedAccountId}`);
                        console.log(`💰 Transfer amount: ${metadata.originalAmount || transaction.amount} ${metadata.originalCurrency || transaction.currency} cents`);
                    }
                    // Send push notification to user
                    const userId = transaction.userId;
                    ...
                    try {
                        const notificationResult = await sendPaymentReceivedNotification(userId, displayAmount, displayCurrency, customerName, env);
                        if (notificationResult.success) {
                            console.log(`📱 Push notification sent to user ${userId} for ${metadata.paymentType || "direct"} invoice payment ${invoice.id}`);
                        }
                        else {
                            console.warn(`⚠️ Failed to send push notification for invoice payment ${invoice.id}:`, notificationResult.error);
                        }
                    }
                    catch (notificationError) {
