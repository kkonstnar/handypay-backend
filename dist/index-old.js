...
app.get("/test-db", async (c) => {
    try {
        ...
        console.log("DATABASE_URL available:", !!c.env.DATABASE_URL);
...
app.post("/api/payouts/generate-automatic", async (c) => {
    ...
    `);
        ...
        for (const userBalance of usersWithBalance || []) {
            ...
            const availableBalance = parseFloat(String(userBalance.available_balance)) / 100; // Convert from cents
            // Get payout rules
            ...
            const shouldProcessPayout = await shouldProcessPayoutForUser(String(userId));
            if (shouldProcessPayout) {
                const payout = await createAutomaticPayout(String(userId), availableBalance);
                results.push(payout);
            }
        }
        return c.json({
...
async function shouldProcessPayoutForUser(userId) {
    // Get payout rules
    const rules = await db
        .select()
        .from(payoutRules)
    ...
    if (recentPayout.length === 0) {
        ...
        if (firstTransaction.length === 0)
            return false;
        const daysSinceFirstTransaction = Math.floor((now.getTime() - firstTransaction[0].date.getTime()) /
            (24 * 60 * 60 * 1000));
        return daysSinceFirstTransaction >= rule.firstTransactionDelayDays;
    }
    else {
        // Subsequent payout - check if random delay has passed
        const daysSinceLastPayout = Math.floor((now.getTime() - recentPayout[0].payoutDate.getTime()) /
            (24 * 60 * 60 * 1000));
        const randomDelay = Math.floor(Math.random() *
            (rule.subsequentDelayDaysMax - rule.subsequentDelayDaysMin + 1)) + rule.subsequentDelayDaysMin;
        return daysSinceLastPayout >= randomDelay;
    }
}
// Helper function to create automatic payout
async function createAutomaticPayout(userId, amount) {
    console.log(`💸 Creating automatic payout for user ${userId}, amount: $${amount}`);
