...
const sendOnboardingCompleteNotification = async (env: any, userId: string) => {
  try {
    console.log(
      `🎉 Sending onboarding complete notification to user ${userId}`
    );
    ...
    const expoPushTokens = userTokens.map((t: any) => t.token);
    ...
    if (!expoResponse.ok) {
      ...
      console.error(
        "❌ Failed to send onboarding complete notification:",
        errorText
      );
      return;
    }
    ...
    console.log(
      `✅ Onboarding complete notification sent to ${expoPushTokens.length} devices for user ${userId}`,
      expoResult
    );
  } catch (error) {
...
const sendBanNotification = async (
  ...
  try {
    ...
    const expoResult = await expoResponse.json();
    console.log(
      `✅ Ban notification sent to ${expoPushTokens.length} devices for user ${userId}`,
      expoResult
    );
  } catch (error) {
  ...
  try {
    ...
    if (isBanned) {
      const bannedAccount = await db
        ...
        .where(
          and(
            eq(bannedAccounts.userId, userId),
            eq(bannedAccounts.isActive, true)
          )
        )
        .limit(1);
    ...
    if (userEmail && !banDetails) {
      const emailBan = await db
        ...
        .where(
          and(
            eq(bannedAccounts.email, userEmail),
            eq(bannedAccounts.isActive, true),
            eq(bannedAccounts.banType, "persistent")
          )
        )
        .limit(1);
...
userRoutes.post("/ban/:userId", async (c) => {
  try {
    ...
    if (!userId || !banReason || !banType) {
      return c.json(
        { error: "Missing required fields: userId, banReason, banType" },
        400
      );
    }
...
userRoutes.post("/test-onboarding-complete/:userId", async (c) => {
  try {
    ...
    console.log(
      `🧪 Testing onboarding complete notification for user ${userId}`
    );
    ...
    return c.json(
      { error: "Failed to test onboarding complete notification" },
      500
    );
  }
});
