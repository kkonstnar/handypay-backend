    ...
    return c.json(
      {
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to store push token",
      },
      500
    );
  }
...
pushNotificationRoutes.post("/send", async (c) => {
  try {
    ...
    let sound: "default" | null = "default";
    ...
    const expoResult = await expoResponse.json();
    console.log(
      `✅ Notification sent to ${expoPushTokens.length} devices for user ${userId}`,
      expoResult
    );
